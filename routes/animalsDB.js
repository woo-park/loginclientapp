...
const AnimalNodes = require('../models/AnimalNodes.js')
...
router.get('/', passport.authenticate('jwt', {session: false}), function(req, res) {
  ...
  if (token) {
    ...
    //   res.json(books)
    // });
    AnimalNodes.find({},function(err, nodes){
      if(err) return next(err);
...
router.post('/', function(req, res) {    // /api/auth/register
  if (!req.body) {
    ...
    initialNodes.forEach((each) => {
      AnimalNodes.findOneAndUpdate({id:each.id},{ id: each.id, name: each.name, interests: each.interests }, function(err, result){
        if(err) return next(err);


        if(result === null) {
          console.log(result,'result is null- does not exist before')
          let newNode = new AnimalNodes({
            id: each.id,
            name: each.name,
